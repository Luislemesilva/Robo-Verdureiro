...
|---------------------|-----------|
| Relé da bomba       | 23        |
| DIR da guilhotina   | 18        |
| STEP da guilhotina  | 26        |
| DIR da esteira      | 13        |
| STEP da esteira     | 27        |
...
```
ESP32 GPIO 23  ──►  IN do módulo relé  ──►  Bomba d'água
ESP32 GPIO 18  ──►  DIR  do driver da guilhotina
ESP32 GPIO 26  ──►  STEP do driver da guilhotina
ESP32 GPIO 13  ──►  DIR  do driver da esteira
ESP32 GPIO 27  ──►  STEP do driver da esteira

GND (ESP32)    ──►  GND do driver / relé (terra comum obrigatório)
```

> ⚠️ **Atenção:** o relé é **active LOW**. O firmware mantém o pino em `HIGH` (bomba desligada) e aciona com `LOW`. Verifique se o seu módulo de relé segue esta lógica.
...
| Hash      | Mensagem                                                        |
|-----------|-----------------------------------------------------------------|
| `a3f1c2d` | feat: estrutura inicial do projeto                             |
| `b82e4a1` | feat(esp32): controle da guilhotina com motor de passo         |
| `c91d3b5` | feat(esp32): controle da esteira com motor de passo            |
| `d04c7e8` | feat(esp32): controle do relé da bomba (active low)            |
| `e37b2f0` | feat(esp32): conexão WiFi e loop de 24h                        |
| `f5a9c14` | feat(hf): servidor FastAPI com endpoint /prever                |
| `g61d8b3` | feat(hf): modelo RandomForest de irrigação                     |
| `h74e2a9` | feat(hf): fallback com regra determinística sem modelo.pkl     |
| `i88f5c2` | feat(hf): Dockerfile para deploy no Hugging Face Space         |
| `j92a1d7` | feat(esp32): integração HTTPS com servidor HF via ArduinoJson  |
| `k05b4e1` | feat(esp32): modo teste para desenvolvimento                   |
| `l13c7f6` | fix(esp32): relé active low — inverte lógica de acionamento    |
| `m27d9b4` | fix(hf): resposta JSON padronizada como {"irrigar": "SIM/NAO"} |
| `n41e2c8` | refactor: organiza constantes e parâmetros mecânicos           |
| `o56f1a3` | docs: README completo com pinagem e instruções de deploy       |

> Os hashes acima são ilustrativos. Ao criar o repositório git real, os hashes serão gerados automaticamente.

### Comandos Git Sugeridos

Para inicializar o repositório e recriar um histórico limpo:

```bash
# Inicializar repositório
git init
git remote add origin https://github.com/SEU_USUARIO/robo-verdureiro.git

# Commit inicial
git add esp32/robo_verdureiro.ino
git commit -m "feat: estrutura inicial do projeto"

# Commits incrementais (adicione conforme o desenvolvimento real)
git add huggingface/app.py
git commit -m "feat(hf): servidor FastAPI com endpoint /prever"

git add huggingface/treinar_modelo.py
git commit -m "feat(hf): script de treino do modelo RandomForest"

git add huggingface/Dockerfile huggingface/requirements.txt
git commit -m "feat(hf): Dockerfile para deploy no Hugging Face Space"

git add README.md
git commit -m "docs: README completo com pinagem e instruções de deploy"
...
│
├── esp32/
│   └── robo_verdureiro.ino      ← Firmware principal do ESP32
│
├── huggingface/
│   ├── app.py                   ← API FastAPI (servidor de IA)
│   ├── treinar_modelo.py        ← Treino local do modelo ML
│   ├── modelo.pkl               ← Modelo serializado (gerado via treinar_modelo.py)
│   ├── requirements.txt         ← Dependências Python
...
Este projeto é de uso acadêmico/educacional. Sinta-se livre para adaptar e usar.
